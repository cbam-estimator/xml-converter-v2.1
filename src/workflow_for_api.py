...
from src.create_xml import create_report
import pprint  # noqa
from src.shared import shared_data
import openpyxl
from src.helper import exact_search, non_exact_search
import tempfile

pause_between_reports = True
pause_after_intro = False

def start(report_type: str, customer_file_url: str, supporting_doc_url: str, config_file="config/config_JY.yml"):

    Log.clear()
    
    # Step 1: Load configuration and default data
    try:    
        ...
        Log.info("Configuration and default data loaded.")
    except Exception as e:
        Log.error(f"Failed during configuration or default data loading: {e}")
        raise e


    # Step 2: Load the customer file and data
    try:
        config["input_file"] = customer_file_url
        customer_data = [process_input_file(customer_file_url, config["version_layouts_file"])]
        ...
        importer_name = shared_general_info["importer_name"] = customer_general_info["importer_name"]
        ...
        Log.info(f"Loaded customer file for {importer_name}, Q{quarter}-{year}")

    except Exception as e:
        Log.error(f"Failed while loading or parsing customer file: {e}")
        raise e
    ...
    # Step 4: Determine the report type and load default supporting documents
    try:
        if report_type == "1":
            Log.info(f"Type: zero_report_with_default_docs")
            config["tagset_determination_approach"] = "zero_report_with_default_docs"

            if not supporting_doc_url:
                Log.error("No supporting document URL provided for zero_report_with_default_docs.")
                return

            config["default_supporting_documents"] = [supporting_doc_url]
            Log.info(f"Loaded supporting document from: {supporting_doc_url}")
        
        elif report_type == "2":
            Log.info(f"zero_report_without_docs")
            config["tagset_determination_approach"] = "zero_report_without_docs"
            Log.info("No supporting documents required.")
        
        elif report_type == "3":
            pass
    except Exception as e:
        Log.error(f"Failed during report type setup: {e}")
        raise e

    # Step 5: Load supporting documents from supplier data folder
    try:
        default_quarters = {"Q4-2023", "Q1-2024", "Q2-2024"}
        current_quarter = f"Q{quarter}-{year}"
        has_supporting_docs = bool(config.get("default_supporting_documents"))

        if current_quarter not in default_quarters and not has_supporting_docs:
            load_customer_supporting_documents(importer_name, config)
            Log.info("Loaded supporting documents from supplier data folder.")

    except Exception as e:
        Log.error(f"Failed during report type setup or supporting document handling: {e}")
        raise e
    
    # Step 6: Prepare data, generate report, save outputs
    try:
...
def output_path(config, importer_name, year, quarter):
    base_tmp_dir = tempfile.gettempdir()  # returns '/tmp'

    date = datetime.datetime.now().strftime("%Y-%m-%d")
    importer_name = importer_name.replace("&amp;", "&").replace(" ", "_")
    dir_name = f"{date} - {importer_name} - Q{quarter}-{year}"
    full_output_path = os.path.join(base_tmp_dir, dir_name)


    if not os.path.exists(full_output_path):
        os.makedirs(full_output_path)
        Log.info(f"Created temporary output directory: {full_output_path}")
